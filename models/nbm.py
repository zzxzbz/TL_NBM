"""NBM (Neural Basis Model) core implementation.

Ref: Neural Basis Models for Interpretability.
...
class ConceptNNBasesNary(pl.LightningModule):
...
class ConceptNBMNary(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if nary is None:
            ...
            self._nary_indices = {
                "1": list(combinations(range(self._num_concepts), 1)), 
                "2": list(combinations(range(self._num_concepts), 2))
                }
    ...
    def forward(self, input, feature_mask, return_feats=False):
        bases = []
        masks = []
        for order in self._nary_indices.keys():
            for subnet in range(self._num_subnets):
                input_order = input[:, self._nary_indices[order]]
                subnet_mask = feature_mask[:, self._nary_indices[order]]
                ...
                masks.append(subnet_mask.prod(dim=-1, keepdim=True))
        ...
        out_feats = out_feats * masks.squeeze(-1)
